      ...
      <div class="full-width">
        <q-form action="${url.loginAction}" method="post">
          ...
          <#if isAppInitiatedAction??>
              ...
              <div class="col-1" >
              <q-btn name="cancel-aia" label="${msg('doSubmit')}" type="submit" color="blue-9" :loading="updatedPasswordLoading" @click="updatedPasswordLoading = true" />
            </div>
          <#else>
            <q-btn label="${msg('doApply')}" type="submit" color="blue-9" :loading="updatedPasswordLoading" @click="updatedPasswordLoading = true" />
          </#if>
          </div>   
        </q-form>
      </div>
